...
项目内置 50 条公开文档衍生评测用例。一次真实 API 双版本对照中，候选版本正确性评分由 97.2 提升到 99.6，依据通过率由 96% 提升到 100%，任务完成率由 96% 提升到 100%；同时延迟增加约 29%、Token 增加约 108%、估算成本增加约 97%。

这个结果只说明候选版本需要进入成本风险复核，不能简化成“平均分更高所以直接发布”。完整口径见[真实评测报告](docs/REAL_EVALUATION_REPORT.md)。

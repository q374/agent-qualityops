...
function CasesPage() {
  ...
  return <div className="page-grid">
    ...
    <Panel>
      ...
      {resource.loading ? <LoadingRows /> : resource.error ? <ErrorState message={resource.error} retry={() => void resource.refresh()} /> : !resource.data?.length ? <EmptyState title="评测集还是空的" description="导入带来源、事实卡、参考答案和预期行为的 JSON 用例后即可开始评测。" action={<button className="btn btn-primary" onClick={() => setShowImport(true)}>导入第一批用例</button>} /> : !filtered.length ? <EmptyState title="没有匹配用例" description="调整搜索词或分类筛选条件。" /> : <div className="table-scroll"><table><thead><tr><th>用例</th><th>分类</th><th>风险</th><th>预期行为</th><th>来源与证据</th></tr></thead><tbody>{filtered.map((item) => <tr key={item.id}><td className="case-main"><small>{item.external_id}</small><strong>{item.title}</strong><p>{item.input_text}</p></td><td><Badge tone="blue">{categoryLabels[item.category] || item.category}</Badge></td><td><Badge tone={item.risk_level === 'high' ? 'bad' : item.risk_level === 'medium' ? 'warn' : 'neutral'}>{item.risk_level === 'high' ? '高' : item.risk_level === 'medium' ? '中' : '低'}</Badge></td><td className="muted-cell">{item.expected_behavior}</td><td className="evidence-cell">{item.source_url ? <a className="source-link" href={item.source_url} target="_blank" rel="noreferrer">{item.source_title || '查看来源'} ↗</a> : <span className="muted">未提供</span>}<details><summary>查看合成事实卡</summary><p>{item.evidence_text}</p></details></td></tr>)}</tbody></table></div>}
    </Panel>
...
export default function App() {
  ...
  return <div className="app-shell">
    <aside className={cn('sidebar', mobileNav && 'mobile-open')}>
      ...
      <div className="sidebar-foot"><div className="service"><i className={health.error ? 'offline' : health.loading ? 'checking' : 'online'} /><div><strong>{health.error ? '服务未连接' : health.loading ? '正在检查' : '后端服务正常'}</strong><small>{health.error ? '数据不会被模拟' : health.data?.deepseek_configured ? 'DeepSeek 已配置' : 'Demo 模式 · 未配置 API'}</small></div><button onClick={() => void health.refresh()} aria-label="重新检测"><Icon name="refresh" size={14} /></button></div><p>Agent QualityOps <b>v0.2</b></p></div>
    </aside>
    <div className="main-wrap">
      <header className="topbar"><button className="menu-btn" onClick={() => setMobileNav(!mobileNav)} aria-label="菜单"><span /><span /><span /></button><div className="breadcrumbs"><span>质量运营台</span><b>/</b><strong>{titleFor(page)}</strong></div><div className="topbar-right"><span className="env">{health.data?.deepseek_configured ? 'Demo / DeepSeek 可用' : '离线 Demo 环境'}</span><span className="avatar">ZC</span></div></header>
